...
class SettingsPage
{
	...
	public function add_settings_page()
	{
		...
		add_management_page(
			__('Importar/Exportar Perfis do arquivo','profiles'),
			__('Importar/Exportar Perfis do arquivo','profiles'),
			'import',
...
/**
	...
	{
		// Set class property
		$this->options = get_option( 'profiles_plugin_options', array() );
		?>
        <div class="wrap">
            <h2><?php _e('Configurações do Plugin de Perfis', 'profile') ?></h2>           
            <form id="profiles_csv_import_form" method="post" action="options.php">
            <?php
                // This prints out all hidden setting fields
                settings_fields( 'profiles_option_group' );   
                do_settings_sections( 'profiles-setting-admin' );
                echo '<b>'.__('Arquivo a importar:', 'profiles').'</b>';
                echo '<input id="profiles-import-csv" '
                		. 'name="profiles-import-csv" type="file">';
    ...
    {        
        register_setting(
            'profiles_option_group', // Option group
            'profiles_plugin_options', // Option name
            array( $this, 'sanitize' ) // Sanitize
        );

        
		if(array_key_exists('page', $_REQUEST) && $_REQUEST['page'] == 'profiles-import-file')
		{
			$path = plugin_dir_url(__FILE__) ;
			wp_register_script('profiles_options_scripts', $path . '/js/profiles_options_scripts.js', array('jquery'));
			
			wp_enqueue_script('profiles_options_scripts');
					
			wp_localize_script( 'profiles_options_scripts', 'profiles_options_scripts_object',
    ...
    {
        _e('Configurações personalizadas do Plugin Profiles', 'profiles');
    }
...
if( is_admin() )
    $profiles_settings_page = new \Profile\SettingsPage();
